...
class Json{
    ...
    private $ids = [];
    private $description;
    private $created_at = date("Y-m-d H:i:s");
    private $currentStatus;
    private $masterUsr_id;
    private $slaveUsr_id;
    private $initiated = date("Y-m-d H:i:s");
    private $done = date("Y-m-d H:i:s");
    private $deleted;
    ...
    public function createTask(array $new_task){
        ...
        if($this->number_of_records == 0){
           $this->putJson();
            ...
            if(!in_array($new_task['created_at'], $this->date("Y-m-d H:i:s"))){
                $this->putJson();
    ...
    function updateTask($id, $task, $value){
        foreach($this->tasks as $key => $stored_user){
            if($stored_user['id'] == $user_id){ 
               $this->tasks[$key][$field] = $value;
            if ($value['id'] == $id){ 
